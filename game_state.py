...
class GameState:
    ...
    def clear_lines(self, board):
        """Verifica e limpa linhas completas em um tabuleiro temporário."""
        lines_cleared = 0
        rows_to_clear = [r for r in range(len(board)) if all(cell == 1 for cell in board[r])]
        cols_to_clear = [c for c in range(len(board[0])) if all(board[r][c] == 1 for r in range(len(board)))]
