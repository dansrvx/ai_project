...
class PlayTree:
    ...
    def build_tree(self, node, current_depth):
        ...
        """
        if current_depth >= self.depth or not node.game_controller.piece_sequence.sequence:
            return  # Stop if maximum depth is reached or no pieces left
    ...
    def print_tree(self, node=None, depth=0):
        ...
        if node.piece_name:
            print(
                f"{indent}- Piece: {node.piece_name} at Position: {node.position} (Cost: {node.cost}, Heuristic: {node.heuristic})")
