...
class Game:
    ...
    def find_placement_bfs(self, piece):
        """
        Finds a valid placement for the piece using Breadth-First Search.

        Args:
            piece (list[list[int]]): The piece to place.

        Returns:
            tuple: (row, col) tuple of the top-left position if a valid placement is found,
                   None otherwise.
        """
        piece_rows = len(piece)
        piece_cols = len(piece[0])
        start_row, start_col = 0, 0  # Start searching from top-left corner
        frontier = deque([(start_row, start_col)]) # Queue of positions to check
        visited = set([(start_row, start_col)]) # Keep track of visited positions

        while frontier:
            current_row, current_col = frontier.popleft()

            if self.can_place_piece(piece, current_row, current_col):
                return (current_row, current_col) # Found a valid placement!

            # Explore neighbors: move right and down
            possible_moves = [
            ...
            for next_row, next_col in possible_moves:
                if (0 <= next_row < self.board.rows and 0 <= next_col < self.board.cols and
                    (next_row, next_col) not in visited): # Check board boundaries and if visited
                    visited.add((next_row, next_col))
                    frontier.append((next_row, next_col))

        return None # No valid placement found after BFS
...
class GameGUI:
    def __init__(self, root, game):
        ...
        self.auto_place_button = tk.Button(root, text="Auto Place Piece (BFS)", command=self.auto_place_piece_gui, font=("Arial", 14))
        self.auto_place_button.pack(pady=10)
    ...
    def place_piece_gui(self):
        ...
        if self.game.play_next_piece(row, col):
            ...
            if game_over_status == "victory":
                ...
                self.auto_place_button.config(state=tk.DISABLED) # Disable further play
    ...
    def auto_place_piece_gui(self):
        """Handles automatic piece placement using BFS."""
        next_piece_info = self.game.piece_sequence.peek_next_piece()
        if not next_piece_info:
            return

        next_piece_name, next_piece_shape = next_piece_info
        placement = self.game.find_placement_bfs(next_piece_shape)
        ...
        if placement:
            ...
            if self.game.play_next_piece(row, col):
                ...
                self.update_next_piece_display()
                self.update_score_display()
                self.status_label.config(text=f"Piece placed automatically at Row: {row}, Col: {col} using BFS.")

                game_over_status = self.game.is_game_over()
                if game_over_status == "victory":
                    messagebox.showinfo("Game Over", "Congratulations! You placed all pieces. Victory!\nScore: " + str(self.game.score), font=("Arial", 14))
                    self.place_button.config(state=tk.DISABLED)
                    self.auto_place_button.config(state=tk.DISABLED)
                elif game_over_status == "defeat":
                    messagebox.showinfo("Game Over", "Game Over. No more valid moves. Defeat!\nScore: " + str(self.game.score), font=("Arial", 14))
                    self.place_button.config(state=tk.DISABLED)
                    self.auto_place_button.config(state=tk.DISABLED)
                ...
                self.status_label.config(text="BFS found a location but placement failed. This should not happen.") # Debugging message

        else:
            self.status_label.config(text="BFS could not find a valid placement for this piece. Game might be over.")
            game_over_status = self.game.is_game_over()
            if game_over_status == "defeat":
                messagebox.showinfo("Game Over", "Game Over. No more valid moves. Defeat!\nScore: " + str(self.game.score), font=("Arial", 14))
                self.place_button.config(state=tk.DISABLED)
                self.auto_place_button.config(state=tk.DISABLED)
